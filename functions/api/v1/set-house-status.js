...
import { apiSetHouseStatus } from "../../_shared/housekeeping.js";
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    await requirePermission(env, session, "housekeeping.write");
    const body = await readJson(request);
    if (!body) return json({ error: "请求格式错误" }, 400);
    return json(await apiSetHouseStatus(env, session, body));
  } catch (error) {
