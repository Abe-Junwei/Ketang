...
import { listUsers, createUser, updateUser, deactivateUser, reactivateUser, resetUserPassword } from '../../../_shared/users.js';
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (body.action === 'create') return json(await createUser(env, session, body));
    ...
    if (body.action === 'deactivate') return json(await deactivateUser(env, session, body));
    if (body.action === 'reactivate') return json(await reactivateUser(env, session, body));
    if (body.action === 'reset_password') return json(await resetUserPassword(env, session, body));
    return json({ error: '未知 action' }, 400);
  } catch (error) {
    const status = /登录已过期/.test(error.message) ? 401 : (/管理员/.test(error.message) ? 403 : 400);
    return json({ error: safeErrorMessage(error) }, status);
  }
}
