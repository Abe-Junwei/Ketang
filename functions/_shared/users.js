import { requireAdmin, mustChangePassword, verifySession, verifyPassword, hashPasswordPlain } from './auth.js';
import { insertAudit, queryD1, runD1 } from './d1.js';

const WEAK_PASSWORDS = new Set(['admin', 'zhike', '123456', 'password', '111111']);
const VALID_ROLES = new Set(['admin', 'zhike', 'kitchen', 'housekeeping', 'viewer']);
...
async function bumpAuthVersion(env, userId) {
  ...
  const rows = await queryD1(env, 'SELECT auth_version FROM users WHERE id = ? LIMIT 1', [userId]);
...
export async function createUser(env, session, body) {
  ...
  const existing = await queryD1(env, 'SELECT id FROM users WHERE username = ? LIMIT 1', [username]);
  ...
  await insertAudit(env, '新增用户', 'user', meta.last_row_id, { username, role }, session);
...
export async function updateUser(env, session, body) {
  ...
  const rows = await queryD1(env, 'SELECT * FROM users WHERE id = ? LIMIT 1', [id]);
  ...
  if (hash) {
    ...
    await runD1(env, 'UPDATE users SET display_name=?, role=? WHERE id=?', [displayName, role, id]);
  }
  await insertAudit(env, '更新用户', 'user', id, { username: existing.username, role, password_changed: !!hash }, session);
...
export async function deactivateUser(env, session, body) {
  ...
  const id = parseInt(body.user_id, 10);
  if (!id) throw new Error('缺少用户 ID');
  if (id === (session.id || session.sub)) throw new Error('不能停用当前登录账号');
  const rows = await queryD1(env, 'SELECT * FROM users WHERE id = ? LIMIT 1', [id]);
  const existing = rows[0];
  if (!existing) throw new Error('用户不存在');
  ...
  await runD1(env, 'UPDATE users SET is_active = 0, auth_version = COALESCE(auth_version, 1) + 1 WHERE id = ?', [id]);
  await insertAudit(env, '停用用户', 'user', id, { username: existing.username }, session);
  return { ok: true };
}
...
export async function reactivateUser(env, session, body) {
  ...
  await insertAudit(env, '启用用户', 'user', id, { username: existing.username }, session);
  return { ok: true };
}
...
export async function resetUserPassword(env, session, body) {
  ...
  await insertAudit(env, '重置用户密码', 'user', id, { username: existing.username }, session);
  return { ok: true, auth_version: authVersion };
}

export async function changeUserPassword(env, userId, oldPassword, newPassword) {
  validateNewPassword(newPassword, oldPassword);
  const rows = await queryD1(env, 'SELECT * FROM users WHERE id = ? LIMIT 1', [userId]);
  const user = rows[0];
  if (!user) throw new Error('用户不存在');
  if (!(await verifyPassword(oldPassword || '', user.password))) throw new Error('原密码错误');
  const hash = await hashPasswordPlain(newPassword);
  const authVersion = await bumpAuthVersion(env, userId);
  await runD1(env, 'UPDATE users SET password = ?, must_change_password = 0 WHERE id = ?', [hash, userId]);
  const updated = await queryD1(
    env,
    'SELECT id, username, display_name, role, auth_version FROM users WHERE id = ? LIMIT 1',
    [userId]
