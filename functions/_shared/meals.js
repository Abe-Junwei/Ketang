...
export async function apiSaveMeals(env, session, body) {
  ...
  const map = body.days || {};
  const statements = [
    { sql: 'UPDATE lodgers SET meal_default_breakfast=?, meal_default_lunch=?, meal_default_dinner=? WHERE id=?', params: [defaults.breakfast, defaults.lunch, defaults.dinner, lodgerId] }
  ];
  stayDateRange(l.check_in_date, l.expected_check_out).forEach(date => {
    const vals = map[date] || { breakfast: defaults.breakfast, lunch: defaults.lunch, dinner: defaults.dinner };
    statements.push({
      sql: 'INSERT OR REPLACE INTO meals (lodger_id, date, breakfast, lunch, dinner) VALUES (?, ?, ?, ?, ?)',
      params: [lodgerId, date, vals.breakfast ? 1 : 0, vals.lunch ? 1 : 0, vals.dinner ? 1 : 0]
    });
  });
  await batchD1(env, statements);
  await insertAudit(env, '保存用斋设置', 'lodger', lodgerId, { name: l.name, defaults, affected_dates: Object.keys(map).length }, session);
