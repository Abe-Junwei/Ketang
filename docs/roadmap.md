...
| Phase 2  | 线上多人验收     | 验证多人共享、并发与主流程            | Phase 1       | 后置总验收 |
...
| Phase 11 | 运维与发布工程化 | 白名单发布、巡检、日志、E2E、性能预算 | Phase 1/2/3/5 | 部分完成   |
...
2. **当前主线：P1-2 权限可视化** + **P1-3 巡检/性能基线** + **P2-1 日常规则补强**（功能与运维先做完，再统一验收）。
...
**排期：后置，纳入最终总验收**（2026-07-02 拍板：开发阶段先跳过，等功能项收尾后一次性测）。
