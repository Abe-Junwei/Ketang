...
| 阶段     | 名称           | 目标                               | 依赖      | 状态   |
| -------- | -------------- | ---------------------------------- | --------- | ------ |
| Phase 0  | 冻结在线架构   | 把当前未提交在线增强变成稳定基线   | 已有代码  | 进行中 |
| Phase 1  | 生产环境配置   | 让正式域名使用 D1 权威数据         | Phase 0   | 待执行 |
| Phase 2  | 线上多人验收   | 验证多人共享、并发与主流程         | Phase 1   | 待执行 |
| Phase 3  | 账号与安全收口 | 默认密码、权限、审计、入口安全     | Phase 1   | 待执行 |
| Phase 4  | 公开预约入口   | 外部提交预约，内部审核处理         | Phase 2/3 | 待执行 |
| Phase 5  | 本地数据迁移   | 旧 `ketang.db` 迁移到 D1           | Phase 2/3 | 待执行 |
| Phase 6  | 备份与恢复     | 手动备份可用，后续接 R2 定时备份   | Phase 5   | 待执行 |
| Phase 7  | 基础业务补强   | 校验、报表、房务、用斋、打印等打磨 | Phase 2   | 待执行 |
| Phase 8  | 移动端适配     | 手机浏览器可完成日常核心操作       | Phase 7   | 待执行 |
| Phase 9  | 夏季活动排房   | 活动排房计划、预分房、冲突检查     | Phase 7/8 | 待执行 |
...
- 强制修改 `admin`、`zhike` 默认密码。
- 确认管理员和知客师权限边界。
- 确认普通知客师不能管理用户、恢复备份或执行危险操作。
- 增加或确认登录限流。
- 增加会话过期策略。
...
1. 完成 Phase 0：先把当前在线增强代码和文档提交成稳定版本。
2. 完成 Phase 1-3：把线上环境、多人验收、账号安全跑通。
3. 完成 Phase 5：迁移真实数据，让 D1 成为权威源。
4. 完成 Phase 6：先建立可靠备份。
5. 完成 Phase 4：再开放公开预约入口。
6. 完成 Phase 7：打磨日常客堂使用细节。
7. 完成 Phase 8：补齐移动端日常核心操作。
8. 完成 Phase 9：进入夏季活动排房专项。
9. 最后评估 Phase 10：AI 辅助排房。

公开预约放在迁移前或迁移后都可以，但若寺院近期就需要外部报名入口，可以把 Phase 4 提前到 Phase 3 之后；如果更重视数据稳妥，先迁移和备份再开放公开入口。
...
2. 指定高级知客需要后续在账号模型中落地：可通过独立角色、权限开关或用户标记实现。
