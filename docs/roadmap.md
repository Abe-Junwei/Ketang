...
| Phase 1  | 生产环境配置     | 让正式域名使用 D1 权威数据            | Phase 0       | 基本完成   |
| Phase 2  | 线上多人验收     | 验证多人共享、并发与主流程            | Phase 1       | 进行中     |
| Phase 3  | 账号与安全收口   | 默认密码、权限、审计、入口安全        | Phase 1       | 进行中     |
| Phase 4  | 公开预约入口     | 外部提交预约，内部审核处理            | Phase 3/5/6   | 后置       |
| Phase 5  | 本地数据迁移     | 旧 `ketang.db` 迁移到 D1              | Phase 1/3     | 进行中     |
| Phase 6  | 备份与恢复       | 手动备份可用，后续接 R2 定时备份      | Phase 5       | 进行中     |
...
| Phase 11 | 运维与发布工程化 | 白名单发布、巡检、日志、E2E、性能预算 | Phase 1/2/3/5 | 新增优先级 |
...
1. Phase 1 收尾：确认生产域名、D1 binding、`KETANG_SESSION_SECRET`、发布面阻断均稳定。
2. Phase 5 优先闭环：完成 `ketang.db` 到 D1 的正式迁移，导入后核对房间、床位、在住、房务、用户数量。
3. Phase 6 立即跟进：在真实数据进入 D1 后，完成一次在线 JSON 导出、恢复演练和 D1 控制台备份记录。
4. Phase 2 做线上多人验收：用两端账号跑入住、换床、退房、用斋、CSV 导出、房态刷新。
5. Phase 3 做权限与账号收口：角色权限已具备基础代码，下一步补管理员可视化配置、权限契约测试和离职停用流程。
6. Phase 11 穿插执行：白名单发布目录、发布后巡检、请求 ID、错误码、关键路径 E2E、生产性能预算必须早于公开预约。
7. Phase 7 继续补强：身份证必填、手机号规则、金额校验、房务配置、导出/打印格式等放在真实数据迁移后打磨。
8. Phase 4 后置开放：公开预约等真实数据、备份、权限、巡检稳定后再开放，避免把未稳定后台暴露给外部流量。
9. Phase 8-9 进入体验与活动专项：移动端日常操作先于夏季活动排房；活动排房依赖真实床位和多人验收结果。
10. Phase 10 继续暂缓：先规则引擎与脱敏数据，再评估 AI 辅助排房。

当前排期原则：先把“真实数据 + 可恢复 + 可多人协同 + 可发布巡检”闭环，再做外部入口和复杂活动排房。公开预约不再建议提前到 Phase 3 后立即开放。
...
**脚本/本地：已完成。** `export_ketang_db_to_json.py` + `verify_migration_json.py` 已校验本地权威库：房间 64、床位 522、在住 311，外键与重复占床通过。

**线上：待人工执行。**

目标：确认 `ketang.db` 转换后的 JSON 已完整进入 D1，并成为正式权威数据源。

任务：

- 使用 `data/ketang-cloud-import.json` 在「系统设置 → 从 JSON 恢复」完成一次正式导入。
- 导入后核对房间、床位、在住、房务、用户、schema_version、app_meta 数量。
- 抽查 10 个房间、20 个在住人员、10 个床位状态，确认无错床、漏人、重复占床。
- 导入成功后立即导出一份在线 JSON，作为迁移后基线备份。

验收：

- 线上房态显示与旧库一致。
- 在住 311 人、房间 64 间、床位 522 张等关键数量与迁移包一致或差异有记录。
- 新增入住、换床、退房、历史查询、CSV 导出可用。

#### P0-2 备份恢复闭环

**脚本/本地：已完成。** `compare_backup_json.py` 已对比 `ketang-cloud-import.json` 与 `ketang-restore-20260702.json`，关键表数量一致。

**线上：待人工执行。**

目标：证明真实数据不只“能导入”，还“能恢复”。
...
- 最近一次白名单部署：`4b82fb1b.ketang-6as.pages.dev`（2026-07-02）。

验收：

- 部署命令默认使用白名单目录。✅
- 生产敏感路径巡检全部 404/403（Access 环境下 403 亦视为已阻断）。✅

### 19.3 短期优先级（P1）

#### P1-1 线上多人业务验收

目标：验证真实数据下多人同时操作不会破坏床位状态。

任务：

- 两端同时登录，跑入住、换床、退房、房务、用斋、历史查询。
- 针对同一床位并发分配做一次冲突测试。
- 记录 board-version 刷新是否及时。
...
目标：把已经存在的权限底层能力变成管理员可维护的产品能力。

任务：

- 增加角色权限配置界面。
- 增加前后端权限码一致性测试、默认角色权限快照测试、关键 API 403 回归测试。
- 明确高级知客 `is_advanced` 与角色权限的关系。

验收：

- 管理员修改角色权限后，新会话立即生效。
- 非授权角色既看不到入口，也无法绕过 API 执行。

#### P1-3 发布后巡检与生产性能预算

**部分完成：** `scripts/post_deploy_check.py`、`test_prod_latency.py` 已存在；`test_p0_scripts.py` 覆盖白名单构建。

**待完成：** Cloudflare Access 下自动化巡检需带凭证或改在已登录浏览器验收；read-model 304 命中率与 P50/P95 基线待记录。

目标：每次发布后自动知道“有没有泄露路径、有没有明显变慢”。

任务：

- 固化生产巡检脚本：敏感路径 404、核心静态资源/API 200、`db.js` 版本匹配。
- 固化 `test_prod_latency.py` 或等价脚本，记录登录、session、read-model、board-version 的 P50/P95。
- 为 read-model 304 命中率和登录接口耗时建立基线。

验收：

- 每次部署后 5 分钟内得到巡检结论。
- 认证与 read-model 性能退化有明确阈值。
...
- 公开预约入口：等真实数据迁移、备份恢复、权限和巡检闭环后开放。
- 企业微信通知：等公开预约入口稳定后再接入。
- 夏季活动排房：等真实床位数据、多端验收、日常规则补强后进入专项。
