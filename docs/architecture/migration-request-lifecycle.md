...
客户端（`eventApplyOptimistic` / `infoFinalizeWriteResult`）：在线增删改在 API 返回前即更新列表；create 用临时 id，成功后 `deletions` 替换为服务端真实行。

业务写 API 统一 `ensureBusinessDatabaseReady`；settings 房间/床位/客人 create 与营期同为单 batch + `patchRow`。
