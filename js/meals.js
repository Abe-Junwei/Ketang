...
function reservationMealFlags(r) {
  ...
  if (r.meal_breakfast == null && r.meal_lunch == null && r.meal_dinner == null) {
...
function refreshMealNeedPicker(wrapOrId) {
  ...
  const trigger = wrap.querySelector('.meal-need-picker-trigger');
  if (labelEl) labelEl.textContent = formatMealNeedLabel(meal.breakfast, meal.lunch, meal.dinner);
  if (trigger) {
    trigger.classList.toggle('meal-need-picker-trigger-empty', !meal.breakfast && !meal.lunch && !meal.dinner);
  }
  wrap.querySelectorAll('input[type="checkbox"][data-meal]').forEach(function (cb) {
...
function validateMealNeedPicker(id, clearOnly) {
  ...
  if (errEl) errEl.textContent = ok || clearOnly ? '' : '请选择用斋需求（至少选一餐）';
...
function initMealNeedPickers(root) {
  root = root || document;
  root.querySelectorAll('.meal-need-picker:not([data-meal-picker-init])').forEach(function (wrap) {
    ...
    wrap.querySelectorAll('.meal-need-picker-option').forEach(function (label) {
...
function mealNeedPickerHtml(id, required) {
  ...
  return '<div class="meal-need-picker" id="' + id + '"' + req + '>' +
...
function getLodgerMealDefaults(lodgerOrId) {
  const l = typeof lodgerOrId === 'object' ? lodgerOrId : query('SELECT * FROM lodgers WHERE id=?', [lodgerOrId])[0];
  if (!l) return { breakfast: 1, lunch: 1, dinner: 1 };
  if (l.meal_default_breakfast == null && l.meal_default_lunch == null && l.meal_default_dinner == null) {
    const row = query('SELECT breakfast, lunch, dinner FROM meals WHERE lodger_id=? ORDER BY date LIMIT 1', [l.id])[0];
    if (row) {
      return {
...
function getMealFlagsForDate(lodgerId, date) {
  ...
  const row = query('SELECT breakfast, lunch, dinner FROM meals WHERE lodger_id=? AND date=?', [lodgerId, date])[0];
  if (row) return { breakfast: row.breakfast, lunch: row.lunch, dinner: row.dinner };
  return { breakfast: defaults.breakfast, lunch: defaults.lunch, dinner: defaults.dinner };
}
...
function formatLodgerMealsToday(lodgerId) {
  const defaults = getLodgerMealDefaults(lodgerId);
  if (!defaults.breakfast && !defaults.lunch && !defaults.dinner) return '不用斋';
  ...
  return eating.join('·') + ' <span class="meal-skip-hint">跳' + skipped.join('') + '</span>';
}
...
function getMealDayDetail(date) {
  ...
  function bumpRole(role, bf, lc, dn, countPerson, noEat) {
    ...
    if (!byRoleMap[key]) byRoleMap[key] = { role: key, bf: 0, lc: 0, dn: 0, people: 0, noEat: 0 };
  ...
  inHouse.forEach(function (l) {
    ...
    bumpRole(role, flags.breakfast, flags.lunch, flags.dinner, eatsAny, eatsNone);
  ...
  return {
    ...
    resvLc: resvLc,
    resvDn: resvDn,
    inHouseCount: inHouse.filter(function (l) { return isLodgerInHouseOnDate(l, date); }).length,
    resvCount: resvCount,
    byRole: byRole,
    skipped: skipped
  };
...
function getMealDayByRoom(date) {
  ...
  function bump(roomKey, roomName, bf, lc, dn, countPerson) {
    if (!byRoomMap[roomKey]) {
      byRoomMap[roomKey] = { room_name: roomName, bf: 0, lc: 0, dn: 0, people: 0 };
    }
  ...
  `, [date]).forEach(function (l) {
    if (!isLodgerInHouseOnDate(l, date)) return;
    const flags = getMealFlagsForDate(l.id, date);
    const eatsAny = !!(flags.breakfast || flags.lunch || flags.dinner);
    const roomKey = l.room_id ? ('r' + l.room_id) : 'unassigned';
    bump(roomKey, l.room_name || '未分床', flags.breakfast, flags.lunch, flags.dinner, eatsAny);
  });
...
function mountFormMealNeedPickers() {
  ...
  mounts.forEach(function (pair) {
    ...
    if (el && !document.getElementById(pair[1])) el.innerHTML = mealNeedPickerHtml(pair[1], true);
  });
...
function formatMealDaySubline(detail) {
  ...
  const noEatTotal = (detail.byRole || []).reduce(function (sum, r) { return sum + (r.noEat || 0); }, 0);
...
function renderTodayMealsPanel() {
  ...
  if (descEl) descEl.textContent = formatMealDaySubline(detail) || '统计当日在寺挂单与待入住预约';
  statsEl.innerHTML =
    '<div class="meal-stat meal-stat-bf"><span class="meal-stat-head">' + icon('dawn', 'icon-xs') + '<span class="meal-stat-label">早斋</span></span><span class="meal-stat-num">' + (detail.bf || 0) + '</span><span class="meal-stat-unit">人次</span></div>' +
    '<div class="meal-stat meal-stat-lc"><span class="meal-stat-head">' + icon('sun', 'icon-xs') + '<span class="meal-stat-label">午斋</span></span><span class="meal-stat-num">' + (detail.lc || 0) + '</span><span class="meal-stat-unit">人次</span></div>' +
    '<div class="meal-stat meal-stat-dn"><span class="meal-stat-head">' + icon('moon', 'icon-xs') + '<span class="meal-stat-label">药石</span></span><span class="meal-stat-num">' + (detail.dn || 0) + '</span><span class="meal-stat-unit">人次</span></div>';
...
function renderMealPieSidePanel(rows, key, colors) {
  var itemsHtml = rows.map(function (r, i) {
    return '<li class="meals-role-stat-item">' +
      '<span class="meals-role-stat-dot" style="background:' + escapeHtml(colors[i]) + '"></span>' +
      '<span class="meals-role-stat-name">' + escapeHtml(r.role) + '</span>' +
      '<span class="meals-role-stat-num">' + (r[key] || 0) + '</span>' +
      '</li>';
  }).join('');
  if (!itemsHtml) return '<p class="meals-side-empty">暂无</p>';
  return '<div class="meals-side-panel"><ul class="meals-role-stats">' + itemsHtml + '</ul></div>';
}


function renderMealsPanelCharts(byRole) {
  if (typeof Chart === 'undefined' || typeof createKetangPieChart !== 'function') return;
  destroyKetangChartsByPrefix('meals-panel-');
  var T = getChartTheme();
  var specs = [
    { key: 'bf', canvas: 'chart-meals-bf', chartKey: 'meals-panel-bf', statsId: 'meals-role-stats-bf', title: '早斋' },
    { key: 'lc', canvas: 'chart-meals-lc', chartKey: 'meals-panel-lc', statsId: 'meals-role-stats-lc', title: '午斋' },
    { key: 'dn', canvas: 'chart-meals-dn', chartKey: 'meals-panel-dn', statsId: 'meals-role-stats-dn', title: '药石' }
  ];
  specs.forEach(function (spec) {
    var rows = byRole.filter(function (r) { return (r[spec.key] || 0) > 0; })
      .sort(compareMealRoles);
    var labels = rows.length ? rows.map(function (r) { return r.role; }) : ['暂无'];
    var data = rows.length ? rows.map(function (r) { return r[spec.key]; }) : [1];
    var colors = rows.length ? getChartColors(labels.length) : [T.muted];
    var statsEl = document.getElementById(spec.statsId);
    if (statsEl) statsEl.innerHTML = renderMealPieSidePanel(rows, spec.key, colors);
...
function openMealModal(lodgerId) {
  const l = query(`
    ...
    LEFT JOIN beds b ON b.id=l.bed_id
    LEFT JOIN rooms r ON r.id=b.room_id
    WHERE l.id=?`, [lodgerId])[0];
  if (!l) return;
  const defaults = getLodgerMealDefaults(l);
  const label = escapeHtml((l.room_name || '-') + (l.bed_number ? ' / ' + l.bed_number : ''));
  const modal = document.getElementById('modal');
  document.getElementById('modal-title').textContent = '用斋管理 - ' + escapeHtml(personDisplayName(l));
  setModalBody(`
...
function renderMealGrid(lodgerId) {
  ...
  if (!dates.length) {
    tbody.innerHTML = '<tr><td colspan="4" class="empty-tip">未记录入住日期</td></tr>';
    return;
  }
  const defaults = readMealModalDefaults();
  const meals = {};
  query('SELECT * FROM meals WHERE lodger_id=?', [lodgerId]).forEach(function (m) {
    meals[m.date] = m;
  });
  ...
  tbody.innerHTML = dates.map(function (d) {
    ...
    const flags = stored
      ? { breakfast: stored.breakfast, lunch: stored.lunch, dinner: stored.dinner }
      : { breakfast: defaults.breakfast, lunch: defaults.lunch, dinner: defaults.dinner };
    const cells = MEAL_TYPES.map(function (t) {
      if (!defaults[t]) return '<td class="meal-na">—</td>';
      const skipped = !flags[t];
      return '<td><label class="meal-skip-label' + (skipped ? ' is-skipped' : '') + '">' +
        '<input type="checkbox" class="meal-skip-cb" data-date="' + d + '" data-type="' + t + '"' + (skipped ? ' checked' : '') + ' onchange="this.parentElement.classList.toggle(\'is-skipped\', this.checked)"> 跳过' +
        '</label></td>';
    }).join('');
    const isToday = d === today;
    return '<tr class="meal-day-row' + (isToday ? ' meal-day-today' : '') + '" data-date="' + d + '">' +
      '<td class="meal-date-cell">' + (isToday ? '<span class="meal-today-tag">今</span> ' : '') + escapeHtml(d.slice(5)) + '</td>' +
      cells +
      '</tr>';
  }).join('');
}
...
function mealGridSkipToday() {
  const today = todayStr();
  document.querySelectorAll('.meal-day-row[data-date="' + today + '"] .meal-skip-cb').forEach(function (cb) {
    cb.checked = true;
    const label = cb.parentElement;
  ...
  try {
    await withTransaction(async () => {
      Object.entries(map).forEach(function (entry) {
        const date = entry[0];
        const vals = entry[1];
        run('INSERT OR REPLACE INTO meals (lodger_id, date, breakfast, lunch, dinner) VALUES (?, ?, ?, ?, ?)',
...
function generateMeals(lodgerId, startDate, endDate, breakfast, lunch, dinner) {
  ...
  if (isNaN(start.getTime())) {
    console.warn('generateMeals: 无效的入住日期 | invalid check-in date:', startDate);
    return;
  }
  let end;
  if (endDate) {
    ...
    if (isNaN(end.getTime())) {
      console.warn('generateMeals: 无效的预离日期 | invalid expected checkout date:', endDate);
      return;
    }
  } else {
    end = new Date(start);
  ...
  while (cur <= end && safety < 366) {
    ...
    run('INSERT OR IGNORE INTO meals (lodger_id, date, breakfast, lunch, dinner) VALUES (?, ?, ?, ?, ?)',
      [lodgerId, d, bf, lc, dn]);
    cur.setDate(cur.getDate() + 1);
    safety++;
  }
...
async function ensureLodgerMeals(lodgerId, breakfast, lunch, dinner) {
  ...
  const count = query('SELECT COUNT(*) as c FROM meals WHERE lodger_id=?', [lodgerId])[0]?.c || 0;
  if (count > 0) return;
  const flags = breakfast != null ? { breakfast: breakfast, lunch: lunch, dinner: dinner } : getLodgerMealDefaults(l);
  await generateMeals(lodgerId, l.check_in_date, l.expected_check_out, flags.breakfast, flags.lunch, flags.dinner);
}
...
function getMealSummary(lodgerId) {
  ...
  return { breakfast: bf, lunch: lc, dinner: dn, total: rows.length, defaults: defaults };
}
