function renderHistory() {
  ...
  `;
  const params = [];
  if (start) { sql += " AND l.check_in_date >= ?"; params.push(start); }
  if (end) { sql += " AND l.check_in_date <= ?"; params.push(end); }
  if (room) { sql += " AND (r.name LIKE ? OR b.bed_number LIKE ?)"; params.push('%' + room + '%', '%' + room + '%'); }
  if (role) {
    const roleVals = lodgerRoleMatchValues(role);
    sql += ' AND l.role IN (' + roleVals.map(function () { return '?'; }).join(',') + ')';
  ...
  rows.forEach(r => {
    ...
    const bedLabel = escapeHtml((r.room_name || '-') + (r.bed_number ? '/' + r.bed_number : ''));
  ...
  if (rows.length === 0) {
    tbody.innerHTML = '<tr><td colspan="12" class="empty-tip">无匹配记录</td></tr>';
  }
}
...
function exportCSV() {
  ...
  `);
  const headers = ['序号', '房间/床位', '姓名', '身份', '性别', '手机号', '身份证', '入住日期', '预离日期', '实际离院日期', '状态', '来源', '营期', '班级', '早斋天数', '午斋天数', '药石天数', '押金', '房费', '退款', '净收款', '备注'];
  const lines = ['\uFEFF' + headers.map(csvCell).join(',')];
  rows.forEach((r, idx) => {
    const meal = getMealSummary(r.id);
    const bedLabel = (r.room_name || '') + (r.bed_number ? '/' + r.bed_number : '');
    const pay = query("SELECT COALESCE(SUM(CASE WHEN type='押金' THEN amount ELSE 0 END),0) as deposit, COALESCE(SUM(CASE WHEN type='房费' THEN amount ELSE 0 END),0) as room_fee, COALESCE(SUM(CASE WHEN type='退款' THEN amount ELSE 0 END),0) as refund FROM payments WHERE lodger_id=?", [r.id])[0];
