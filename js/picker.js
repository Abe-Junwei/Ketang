...
function refreshSelectPicker(sel) {
  ...
  if (!wrap) return;
  const trigger = wrap.querySelector('.select-picker-trigger');
  const labelEl = trigger && trigger.querySelector('.select-picker-trigger-text');
...
function upgradeSelects(root) {
  root = root || document;
  root.querySelectorAll('select:not([data-picker-upgraded])').forEach(function (sel) {
    ...
    trigger.innerHTML = '<span class="select-picker-trigger-text"></span>' + icon('chevron', 'icon-xs select-picker-chevron');
...
document.addEventListener('click', function (e) {
  if (!e.target.closest('.select-picker') && !e.target.closest('.meal-need-picker')) {
    closeAllSelectPickers();
  }
});
