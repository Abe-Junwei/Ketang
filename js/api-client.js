...
function apiAuthHeaders() {
  ...
  const token = typeof getRemoteSessionToken === 'function' ? getRemoteSessionToken() : '';
...
async function remoteBatchQuery(queries) {
  const result = await remoteDBRequestAsync({ action: 'batch_query', queries: queries });
...
async function apiSetHouseStatus(payload) {
  return apiFetch('/api/v1/set-house-status', { method: 'POST', body: payload });
}

async function apiUpsertReservation(payload) {
  return apiFetch('/api/v1/upsert-reservation', { method: 'POST', body: payload });
}

async function apiUpdateReservationStatus(payload) {
  return apiFetch('/api/v1/reservation-status', { method: 'POST', body: payload });
}

async function apiBatchEventMembers(payload) {
  return apiFetch('/api/v1/batch-event-members', { method: 'POST', body: payload });
}
...
async function apiChangePassword(oldPassword, newPassword) {
  return remoteDBRequestAsync({ action: 'change_password', old_password: oldPassword, new_password: newPassword });
}
...
async function apiImportJsonBackup(tables) {
  return apiFetch('/api/v1/admin/data-backup', { method: 'POST', body: { confirm: true, tables: tables } });
}
