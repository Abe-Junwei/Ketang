...
function populateReportEventSelect() {
  ...
  if (!sel) return;
  const events = query("SELECT id, name, event_type FROM events WHERE status != '已取消' ORDER BY start_date DESC, id DESC");
...
function renderMealReport() {
  ...
  if (!date) { container.innerHTML = '<p class="empty-tip">请选择日期</p>'; return; }
  ...
  const makeTable = (rows, cols) => {
    ...
    return `<table><thead><tr>${cols.map(c => `<th>${c.label}</th>`).join('')}</tr></thead><tbody>` +
      rows.map(r => `<tr>${cols.map(c => `<td>${c.key === 'role' || c.key === 'room_name' ? escapeHtml(r[c.key] || '-') : (r[c.key] || 0)}</td>`).join('')}</tr>`).join('') +
      '</tbody></table>';
  };
  ...
  container.innerHTML = `
    ...
    <h3>按身份汇总</h3>
    ${makeTable(byRole, [{key:'role',label:'身份'}, {key:'people',label:'用餐'}, {key:'noEat',label:'不用斋'}, {key:'bf',label:'早斋'}, {key:'lc',label:'午斋'}, {key:'dn',label:'药石'}])}
    <h3 style="margin-top: var(--space-4);">在寺跳过</h3>
    ${dayDetail.skipped.length ? '<ul class="meals-skip-list">' + dayDetail.skipped.map(function (s) {
      return '<li><span class="meals-skip-name">' + escapeHtml(s.displayName || personDisplayName(s)) + '</span>（' + escapeHtml(lodgerRoleDisplayName(s.role)) + '）跳' + escapeHtml(s.skipped.join('、')) + '</li>';
    }).join('') + '</ul>' : '<p class="empty-tip">无跳过记录</p>'}
    <h3 style="margin-top: var(--space-4);">按房间汇总（在寺）</h3>
    ${makeTable(byRoom, [{key:'room_name',label:'房间'}, {key:'people',label:'用餐人数'}, {key:'bf',label:'早斋'}, {key:'lc',label:'午斋'}, {key:'dn',label:'药石'}])}
  `;
  ...
  if (byRole.length) {
    createKetangChart('report-meal-role', 'chart-meal-role', {
      ...
      data: {
        ...
        datasets: [
          { label: '早斋', data: byRole.map(r => r.bf || 0), backgroundColor: T.bf },
          { label: '午斋', data: byRole.map(r => r.lc || 0), backgroundColor: T.lc },
          { label: '药石', data: byRole.map(r => r.dn || 0), backgroundColor: T.dn }
        ]
      },
      options: {
  ...
  if (byRoom.length && byRoom.length <= 16) {
    createKetangChart('report-meal-room', 'chart-meal-room', {
      ...
      data: {
        ...
        datasets: [
          { label: '早斋', data: byRoom.map(r => r.bf || 0), backgroundColor: T.bf },
          { label: '午斋', data: byRoom.map(r => r.lc || 0), backgroundColor: T.lc },
          { label: '药石', data: byRoom.map(r => r.dn || 0), backgroundColor: T.dn }
        ]
      },
      options: {
        plugins: { legend: { position: 'top' } },
...
function exportMealReportCSV() {
  ...
  const byRole = getMealReportByRole(date);
  const lines = ['\uFEFF' + ['身份', '用餐', '不用斋', '早斋', '午斋', '药石'].map(csvCell).join(',')];
  byRole.forEach(r => lines.push([r.role || '未分类', r.people || 0, r.noEat || 0, r.bf || 0, r.lc || 0, r.dn || 0].map(csvCell).join(',')));
...
function renderDailyReport() {
  ...
  const checkins = query("SELECT COUNT(*) as c FROM lodgers WHERE check_in_date = ? AND status IN ('在住','已退')", [date])[0].c;
  const checkouts = query("SELECT COUNT(*) as c FROM lodgers WHERE actual_check_out = ? AND status IN ('在住','已退')", [date])[0].c;
  const inHouse = query("SELECT COUNT(*) as c FROM lodgers WHERE check_in_date <= ? AND status = '在住' AND (expected_check_out IS NULL OR expected_check_out > ?)", [date, date])[0].c;
  const expectedCheckout = query("SELECT COUNT(*) as c FROM lodgers WHERE status = '在住' AND expected_check_out = ?", [date])[0].c;
  ...
  renderDailyReportCharts(checkins, checkouts, inHouse, expectedCheckout, payMap, meals);
}


function renderDailyReportCharts(checkins, checkouts, inHouse, expectedCheckout, payMap, meals) {
  if (typeof Chart === 'undefined') return;
  const T = getChartTheme();
  ...
  const payData = [];
  const payColors = [];
  [['押金', T.success], ['房费', T.primary], ['退款', T.depart]].forEach(function (pair) {
  ...
  if (payData.length) {
    createKetangChart('report-daily-pay', 'chart-daily-pay', {
      type: 'doughnut',
      data: { labels: payLabels, datasets: [{ data: payData, backgroundColor: payColors, borderWidth: 1 }] },
...
function renderReportTable(rows, cols) {
  ...
  return `<table><thead><tr>${cols.map(c => `<th>${c}</th>`).join('')}</tr></thead><tbody>` +
    rows.map(r => `<tr><td>${escapeHtml(personDisplayName(r))}</td><td>${escapeHtml(lodgerRoleDisplayName(r.role) || '-')}</td><td>${escapeHtml((r.room_name || '-') + (r.bed_number ? '/' + r.bed_number : ''))}</td><td>${escapeHtml(r.phone || '-')}</td></tr>`).join('') +
    '</tbody></table>';
}
...
function exportDailyReportCSV() {
  ...
  `, [date, date]);
  const lines = ['\uFEFF' + ['类型', '房间/床位', '姓名', '身份', '性别', '手机号', '入住日期', '实际离院日期', '备注'].map(csvCell).join(',')];
  rows.forEach(r => {
    const type = r.check_in_date === date ? '入住' : '退房';
    lines.push([type, (r.room_name || '') + (r.bed_number ? '/' + r.bed_number : ''), personDisplayName(r), r.role || '', r.gender || '', r.phone || '', r.check_in_date || '', r.actual_check_out || '', r.notes || ''].map(csvCell).join(','));
  });
  const blob = new Blob([lines.join('\n')], { type: 'text/csv;charset=utf-8;' });
...
function renderMonthlyReport() {
  ...
  if (!month) { container.innerHTML = '<p class="empty-tip">请选择月份</p>'; return; }

  const checkins = query("SELECT COUNT(*) as c FROM lodgers WHERE check_in_date LIKE ? AND status IN ('在住','已退')", [month + '%'])[0].c;
  const checkouts = query("SELECT COUNT(*) as c FROM lodgers WHERE actual_check_out LIKE ? AND status IN ('在住','已退')", [month + '%'])[0].c;
  const payments = query(`
...
function renderEventReport() {
  ...
  members.forEach(m => {
    ...
    if (!groups[key]) groups[key] = { count: 0, male: 0, female: 0, checkedIn: 0, reserved: 0 };
    groups[key].count++;
  ...
  const groupLabel = groupBy === 'role' ? '身份' : groupBy === 'class_name' ? '班级' : '性别';
...
function renderEventReportCharts(groupNames, groups) {
  ...
  createKetangChart('report-event-gender', 'chart-event-gender', {
    ...
    data: {
      ...
      datasets: [
        { label: '男', data: groupNames.map(k => groups[k].male), backgroundColor: T.male, stack: 'gender' },
        { label: '女', data: groupNames.map(k => groups[k].female), backgroundColor: T.female, stack: 'gender' }
      ]
    },
    options: {
  ...
  createKetangChart('report-event-status', 'chart-event-status', {
    ...
    data: {
      labels: groupNames.map(k => k),
      datasets: [
        { label: '已入住', data: groupNames.map(k => groups[k].checkedIn), backgroundColor: T.registered, stack: 'status' },
        { label: '仅预约', data: groupNames.map(k => groups[k].reserved), backgroundColor: T.gap, stack: 'status' }
      ]
    },
    options: {
      scales: { x: { stacked: true }, y: { stacked: true, ticks: { stepSize: 1 } } }
...
function exportEventReportCSV() {
  ...
  const members = query(sql, params);
  const headers = ['姓名 / 法名', '性别', '身份', '班级', '类型', '状态', '入住/预计入住', '预离'];
  const lines = ['\uFEFF' + headers.map(csvCell).join(',')];
  members.forEach(m => {
    lines.push([
      personDisplayName(m), m.gender || '', m.role || '', m.class_name || '',
      m.kind, m.status, m.date_in || '', m.date_out || ''
    ].map(csvCell).join(','));
  });
