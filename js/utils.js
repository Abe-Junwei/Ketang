...
function setTheme(t) {
  ...
  try { localStorage.setItem('ketang-theme', t); } catch (e) { /* ignore */ }
}
...
const LODGER_ROLE_OPTIONS = [
  '法师', '沙弥', '行者', '老师', '管理员', '义工', '营务', '学员', '客人', '工人'
];
...
function lodgerRoleSortIndex(role) {
  ...
  if (idx >= 0) return idx;
  return canon === '未分类' ? LODGER_ROLE_OPTIONS.length + 1 : LODGER_ROLE_OPTIONS.length;
}
...
function compareLodgerRoles(a, b) {
  ...
  return lodgerRoleDisplayName(roleA).localeCompare(lodgerRoleDisplayName(roleB), 'zh-CN');
}
...
function roleSelectOptionsHtml(selectedValue, emptyLabel) {
  ...
  let html = showEmpty ? '<option value="">' + escapeHtml(label) + '</option>' : '';
  ...
  LODGER_ROLE_OPTIONS.forEach(function (role) {
    ...
    html += '<option value="' + escapeHtml(role) + '"' + sel + '>' + escapeHtml(role) + '</option>';
  });
  if (selectedValue && !LODGER_ROLE_OPTIONS.includes(selectedValue) && lodgerRoleCanon(selectedValue) === selectedValue) {
    html += '<option value="' + escapeHtml(selectedValue) + '" selected>' + escapeHtml(selectedValue) + '（旧）</option>';
  }
  return html;
}
...
function spareRoomExcludeClause(alias) {
  alias = alias || 'r';
  return "(COALESCE(" + alias + ".location, '') NOT LIKE '备用%' AND COALESCE(" + alias + ".location, '') NOT LIKE '%备用床%' AND COALESCE(" + alias + ".name, '') NOT LIKE '备用%')";
}
...
function generateSalt(len) {
  ...
  if (cryptoObj && cryptoObj.getRandomValues) {
    ...
    for (let i = 0; i < len; i++) salt += chars[Math.floor(Math.random() * chars.length)];
  }
  return salt;
}
