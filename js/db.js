...
async function initSqlite() {
  ...
  if (!response.ok) {
    throw new Error(`无法加载 SQLite 引擎 (${wasmPath})，HTTP 状态：${response.status}。请确认 lib/sql-wasm.wasm 文件存在且服务从项目根目录启动。`);
  }
...
async function loadDB() {
  ...
  try {
    idb = await Promise.race([
      openIDB(),
      new Promise((_, reject) => setTimeout(() => reject(new Error('IndexedDB 打开超时')), 3000))
    ]);
  } catch (e) {
...
function migrateV1toV2() {
  ...
  db.run("DELETE FROM schema_version WHERE version > (SELECT MIN(version) FROM schema_version)");
  ...
  try { db.run("ALTER TABLE rooms ADD COLUMN location TEXT"); } catch (e) { /* 已存在则忽略 | ignore if exists */ }
  // V1 表可能已有 location 但无 capacity（部分迁移残留），补齐 capacity 列做兜底 | Add capacity column if missing (partial migration residue)
  try { db.run("ALTER TABLE rooms ADD COLUMN capacity INTEGER DEFAULT 1"); } catch (e) { /* 已存在则忽略 | ignore if exists */ }
  ...
  if (existingBeds === 0) {
    rooms.forEach(r => {
      ...
      for (let i = 1; i <= cap; i++) {
        run("INSERT INTO beds (room_id, bed_number, status) VALUES (?, ?, '可用')", [r.id, i + '号床']);
    ...
    const existingBedsList = query("SELECT id, room_id FROM beds ORDER BY room_id, id");
  ...
  lodgers.forEach(l => {
    ...
    run(`INSERT INTO lodgers_new
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
      l.id, l.name, l.dharma_name, l.gender, l.phone, l.id_card,
      l.check_in_date, l.expected_check_out, l.actual_check_out,
      bedId, l.role || null, l.status, l.notes, l.created_at
    ]);
  });

  // 迁移完成前预检：确保 lodgers_new 有数据（防止空迁移破坏数据）| Pre-check: ensure lodgers_new has data before destroying old table
  const newCount = db.exec("SELECT COUNT(*) as c FROM lodgers_new")[0]?.values[0][0] || 0;
  const oldCount = db.exec("SELECT COUNT(*) as c FROM lodgers")[0]?.values[0][0] || 0;
  if (newCount < oldCount && oldCount > 0) {
    throw new Error(`挂单数据迁移不完整：旧表${oldCount}条，新表仅${newCount}条。请重置数据库。`);
  }
...
function migrateV3toV4() {
  ...
  try { db.run("ALTER TABLE lodgers ADD COLUMN guest_id INTEGER"); } catch (e) { /* 已存在则忽略 */ }
  try { db.run("ALTER TABLE lodgers ADD COLUMN source TEXT"); } catch (e) { /* 已存在则忽略 */ }
  try { db.run("ALTER TABLE lodgers ADD COLUMN group_code TEXT"); } catch (e) { /* 已存在则忽略 */ }

  // 3. 从现有 lodgers 生成 guests 主数据，并回写 guest_id（按手机号/身份证去重）
  const lodgers = query("SELECT * FROM lodgers WHERE guest_id IS NULL ORDER BY id");
  const guestMap = {}; // key: phone 或 id_card 或 name
  lodgers.forEach(l => {
    const key = l.phone || l.id_card || `${l.name}|${l.dharma_name || ''}`;
    let guestId = guestMap[key];
    if (!guestId) {
      run(`INSERT INTO guests (name, dharma_name, gender, phone, id_card, visit_count, last_visit_date, notes, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        l.name, l.dharma_name || null, l.gender || null, l.phone || null, l.id_card || null,
        1, l.check_in_date || null, l.notes || null, new Date().toISOString()
      ]);
      guestId = db.exec("SELECT last_insert_rowid() as id")[0].values[0][0];
      guestMap[key] = guestId;
    } else {
      // 累计到访次数
      run("UPDATE guests SET visit_count = visit_count + 1, last_visit_date = COALESCE(?, last_visit_date), updated_at = ? WHERE id = ?",
  ...
  const existingHk = (query("SELECT COUNT(*) as c FROM housekeeping")[0]?.c || 0);
  if (existingHk === 0) {
    ...
    beds.forEach(b => {
      ...
      if (b.status === '停用') run("UPDATE beds SET status='维修' WHERE id=?", [b.id]);
      run("INSERT INTO housekeeping (bed_id, status, notes) VALUES (?, ?, 'V4 初始化')", [b.id, hkStatus]);
    });
  }
...
function migrateV4toV5() {
  ...
  try {
    ...
    try { db.run("ALTER TABLE rooms ADD COLUMN dorm_type TEXT DEFAULT '不限'"); } catch (e) { /* 已存在则忽略 */ }
...
function migrateV5toV6() {
  ...
  existingEvents.forEach(e => { eventMap[e.name] = e.id; });
  ...
  const reservationsHasGroupCode = tableHasColumn('reservations', 'group_code');

  let groups = [];
  if (lodgersHasGroupCode || reservationsHasGroupCode) {
    const parts = [];
    if (lodgersHasGroupCode) parts.push("SELECT group_code FROM lodgers WHERE group_code IS NOT NULL AND group_code != ''");
    if (reservationsHasGroupCode) parts.push("SELECT group_code FROM reservations WHERE group_code IS NOT NULL AND group_code != ''");
  ...
  groups.forEach(g => {
    ...
    run(`INSERT INTO events (name, event_type, gender_type, status, notes) VALUES (?, ?, '混合', '进行中', ?)`,
  ...
  lodgers.forEach(l => {
    run(`INSERT INTO lodgers_new
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
      l.id, l.guest_id || null, (lodgersHasGroupCode ? eventMap[l.group_code] : null) || null, l.name, l.dharma_name || null,
      l.gender || null, l.phone || null, l.id_card || null, l.check_in_date || null,
      l.expected_check_out || null, l.actual_check_out || null, l.bed_id || null,
      l.role || null, (lodgersHasGroupCode ? eventClassMap[l.group_code] : null) || null, l.status || '在住', l.source || null, l.notes || null, l.created_at || null
    ]);
  });
  const newLodgerCount = db.exec("SELECT COUNT(*) as c FROM lodgers_new")[0]?.values[0][0] || 0;
  const oldLodgerCount = db.exec("SELECT COUNT(*) as c FROM lodgers")[0]?.values[0][0] || 0;
  if (newLodgerCount < oldLodgerCount && oldLodgerCount > 0) {
    throw new Error(`挂单数据迁移不完整：旧表${oldLodgerCount}条，新表仅${newLodgerCount}条。`);
  }
  db.run("DROP TABLE lodgers");
  db.run("ALTER TABLE lodgers_new RENAME TO lodgers");
  ...
  reservations.forEach(r => {
    run(`INSERT INTO reservations_new
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
      r.id, r.guest_id || null, (reservationsHasGroupCode ? eventMap[r.group_code] : null) || null, r.name, r.dharma_name || null,
      r.gender || null, r.phone || null, r.id_card || null, r.role || null, (reservationsHasGroupCode ? eventClassMap[r.group_code] : null) || null,
      r.expected_check_in || null, r.expected_check_out || null, r.room_preference || null,
      r.source || null, r.status || '预约', r.notes || null, r.created_at || null
    ]);
  });
  const newResvCount = db.exec("SELECT COUNT(*) as c FROM reservations_new")[0]?.values[0][0] || 0;
  const oldResvCount = db.exec("SELECT COUNT(*) as c FROM reservations")[0]?.values[0][0] || 0;
  if (newResvCount < oldResvCount && oldResvCount > 0) {
    throw new Error(`预约数据迁移不完整：旧表${oldResvCount}条，新表仅${newResvCount}条。`);
  }
...
function migrateV6toV7() {
  ...
  if (count === 0) {
    run("INSERT INTO users (username, display_name, role, password) VALUES (?, ?, ?, ?)", ['admin', '管理员', 'admin', 'sha256$ketang_default_salt$8d62959035f9b60a02e709f9826f3f996d07a09a4f5091e2884642fa01adf8a3']);
    run("INSERT INTO users (username, display_name, role, password) VALUES (?, ?, ?, ?)", ['zhike', '知客师', 'zhike', 'sha256$ketang_default_salt$fc286955fb12bec3fb16b4f2619f9b675337b1240537bc21d830b5f495121565']);
  }
...
function migrateV7toV8() {
  ...
  try {
    ...
    const hasActualCount = cols.values.some(row => row[colIdx] === 'actual_count');
    if (hasActualCount) {
      try {
...
function migrateV8toV9() {
  ...
  try {
    // 为用户表增加软删除标记
    try { db.run("ALTER TABLE users ADD COLUMN is_active INTEGER DEFAULT 1"); } catch (e) { /* 已存在则忽略 */ }
...
function migrateV9toV10() {
  ...
  try {
    ...
    db.run("UPDATE rooms SET dorm_type = '不限' WHERE dorm_type NOT IN ('男寮','女寮','不限') OR dorm_type IS NULL");
    db.run("UPDATE events SET gender_type = '混合' WHERE gender_type NOT IN ('男众','女众','混合') OR gender_type IS NULL");
    db.run("UPDATE events SET status = '筹备中' WHERE status NOT IN ('筹备中','招生中','进行中','已结束','已取消') OR status IS NULL");
    db.run("UPDATE beds SET status = '可用' WHERE status NOT IN ('可用','占用','维修','备用') OR status IS NULL");
    db.run("UPDATE lodgers SET status = '在住' WHERE status NOT IN ('在住','已退','已取消','No-show') OR status IS NULL");
    db.run("UPDATE reservations SET status = '预约' WHERE status NOT IN ('预约','已确认','已入住','已取消','No-show') OR status IS NULL");
    db.run("UPDATE payments SET type = '其他' WHERE type NOT IN ('押金','房费','退款','其他') OR type IS NULL");
    db.run("UPDATE housekeeping SET status = '净房' WHERE status NOT IN ('脏房','净房','查房','可用','占用','维修') OR status IS NULL");
    db.run("UPDATE meals SET breakfast = 0 WHERE breakfast NOT IN (0,1) OR breakfast IS NULL");
    db.run("UPDATE meals SET lunch = 0 WHERE lunch NOT IN (0,1) OR lunch IS NULL");
    db.run("UPDATE meals SET dinner = 0 WHERE dinner NOT IN (0,1) OR dinner IS NULL");
    ...
    `);
    db.run(`INSERT INTO beds_new (id, room_id, bed_number, status, notes, created_at) SELECT id, room_id, bed_number, status, notes, created_at FROM beds`);
    ...
    `);
    db.run(`INSERT INTO meals_new (id, lodger_id, date, breakfast, lunch, dinner, notes) SELECT id, lodger_id, date, breakfast, lunch, dinner, notes FROM meals`);
    ...
    `);
    db.run(`INSERT INTO housekeeping_new (id, bed_id, status, operator, changed_at, notes) SELECT id, bed_id, status, operator, changed_at, notes FROM housekeeping`);
...
function dbTableHasColumn(table, column) {
  ...
  if (!info) return false;
  const idx = info.columns.indexOf('name');
  return info.values.some(function (row) { return row[idx] === column; });
}
...
function migrateV10toV11() {
  ...
  try {
    if (!dbTableHasColumn('reservations', 'meal_breakfast')) {
      db.run('ALTER TABLE reservations ADD COLUMN meal_breakfast INTEGER DEFAULT 1 CHECK(meal_breakfast IN (0,1))');
    }
    if (!dbTableHasColumn('reservations', 'meal_lunch')) {
      db.run('ALTER TABLE reservations ADD COLUMN meal_lunch INTEGER DEFAULT 1 CHECK(meal_lunch IN (0,1))');
    }
    if (!dbTableHasColumn('reservations', 'meal_dinner')) {
      db.run('ALTER TABLE reservations ADD COLUMN meal_dinner INTEGER DEFAULT 0 CHECK(meal_dinner IN (0,1))');
    }
    db.run('UPDATE reservations SET meal_breakfast = 1 WHERE meal_breakfast IS NULL');
...
function migrateV11toV12() {
  ...
  try {
    if (!dbTableHasColumn('lodgers', 'meal_default_breakfast')) {
      db.run('ALTER TABLE lodgers ADD COLUMN meal_default_breakfast INTEGER DEFAULT 1 CHECK(meal_default_breakfast IN (0,1))');
    }
    if (!dbTableHasColumn('lodgers', 'meal_default_lunch')) {
      db.run('ALTER TABLE lodgers ADD COLUMN meal_default_lunch INTEGER DEFAULT 1 CHECK(meal_default_lunch IN (0,1))');
    }
    if (!dbTableHasColumn('lodgers', 'meal_default_dinner')) {
      db.run('ALTER TABLE lodgers ADD COLUMN meal_default_dinner INTEGER DEFAULT 0 CHECK(meal_default_dinner IN (0,1))');
    }
    db.run('UPDATE lodgers SET meal_default_breakfast = 1 WHERE meal_default_breakfast IS NULL');
    db.run('UPDATE lodgers SET meal_default_lunch = 1 WHERE meal_default_lunch IS NULL');
    db.run('UPDATE lodgers SET meal_default_dinner = 1 WHERE meal_default_dinner IS NULL');
    query('SELECT id FROM lodgers').forEach(function (row) {
      const sample = query('SELECT breakfast, lunch, dinner FROM meals WHERE lodger_id=? ORDER BY date LIMIT 1', [row.id])[0];
      if (sample) {
        run('UPDATE lodgers SET meal_default_breakfast=?, meal_default_lunch=?, meal_default_dinner=? WHERE id=?',
          [sample.breakfast ? 1 : 0, sample.lunch ? 1 : 0, sample.dinner ? 1 : (sample.breakfast && sample.lunch ? 1 : 0), row.id]);
      }
    });
...
async function seedRooms() {
  ...
  for (let f = 1; f <= 2; f++) {
    for (let r = 1; r <= 6; r++) {
      ...
      run("INSERT INTO rooms (id, name, location, floor, dorm_type, notes) VALUES (?, ?, ?, ?, ?, ?)",
        [id, name, '客堂' + f + '楼', f, dorm, '']);
      for (let b = 1; b <= 2; b++) {
        run("INSERT INTO beds (room_id, bed_number, status) VALUES (?, ?, '可用')", [id, b + '号床']);
        const bedId = db.exec("SELECT last_insert_rowid() as id")[0].values[0][0];
        run("INSERT INTO housekeeping (bed_id, status, notes) VALUES (?, ?, '新床位初始化')", [bedId, '净房']);
      }
    }
  }
  await saveDB();
...
function safeParams(params) {
  ...
  for (let i = 0; i < params.length; i++) {
    if (params[i] === undefined) { hasUndefined = true; break; }
  }
...
async function resetDatabase() {
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      req.onerror = e => reject(e.target.error);
      req.onblocked = () => console.warn('数据库删除被阻塞，请关闭其他标签页 | Database deletion blocked, close other tabs');
    });
  } catch (e) {
