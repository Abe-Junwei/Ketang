...
function openRoomModal(id) {
  ...
  document.getElementById('modal-title').textContent = isEdit ? '编辑房间' : '新增房间';
  setModalBody(`
...
async function submitRoom(id) {
  ...
  const dup = query('SELECT id FROM rooms WHERE name = ? AND id IS NOT ?', [name, id || 0])[0];
  if (dup) {
...
async function deleteRoom(id) {
  ...
  const bedCount = (query('SELECT COUNT(*) as c FROM beds WHERE room_id = ?', [id])[0]?.c) || 0;
  if (bedCount > 0) {
    return infoToast(`该房间下还有 ${bedCount} 张床位，请先删除床位后再删除房间`);
  }
...
function openBedModal(id) {
  ...
  let occupied = false;
  if (isEdit) {
    const row = query('SELECT b.*, (SELECT COUNT(*) FROM lodgers WHERE bed_id = b.id AND status = \'在住\') AS occupant_count FROM beds b WHERE b.id = ?', [id])[0];
  ...
  document.getElementById('modal-title').textContent = isEdit ? '编辑床位' : '新增床位';
  setModalBody(`
...
async function submitBed(id) {
  ...
  const dup = query('SELECT id FROM beds WHERE room_id = ? AND bed_number = ? AND id IS NOT ?',
    [roomId, number, id || 0])[0];
  if (dup) {
    infoShowFieldError('info-bed-number', '该房间下已存在相同床位号');
    return;
  }
  const occupantCount = id ? (query("SELECT COUNT(*) as c FROM lodgers WHERE bed_id = ? AND status = '在住'", [id])[0]?.c || 0) : 0;
  if (occupantCount > 0 && (status === '维修' || status === '备用')) {
    infoShowFieldError('info-bed-status', '该床位当前有住客，不能设为维修或备用');
    return;
  }
  ...
  try {
    await withTransaction(async () => {
      if (id) {
        ...
        run('UPDATE beds SET room_id=?, bed_number=?, status=?, notes=? WHERE id=?',
...
function openGuestModal(id) {
  const isEdit = !!id;
  let g = { name: '', dharma_name: '', gender: '男', phone: '', id_card: '', emergency_contact: '', emergency_phone: '', notes: '' };
  if (isEdit) {
    const row = query('SELECT * FROM guests WHERE id = ?', [id])[0];
    if (!row) return infoToast('住客档案不存在');
    g = row;
  }
  document.getElementById('modal-title').textContent = isEdit ? '编辑住客档案' : '新增住客档案';
  setModalBody(`
...
async function submitGuest(id) {
  ...
  const dupPhone = phone ? query('SELECT id FROM guests WHERE phone = ? AND phone <> "" AND id IS NOT ?', [phone, id || 0])[0] : null;
  if (dupPhone) {
    infoShowFieldError('info-guest-phone', '该手机号已存在');
    return;
  }
  const dupIdCard = idCard ? query('SELECT id FROM guests WHERE id_card = ? AND id_card <> "" AND id IS NOT ?', [idCard, id || 0])[0] : null;
  if (dupIdCard) {
    ...
    await withTransaction(async () => {
      if (id) {
        run(`UPDATE guests SET name=?, dharma_name=?, gender=?, phone=?, id_card=?,
             ...
             WHERE id=?`,
          [name, person.dharma_name, gender, phone, idCard, emergency, emergencyPhone, notes, now, id]);
...
async function deleteGuest(id) {
  ...
  const refCount = (query('SELECT COUNT(*) as c FROM lodgers WHERE guest_id = ?', [id])[0]?.c) || 0;
  if (refCount > 0) {
...
function renderLodgerList() {
  ...
  lodgers.forEach(l => {
    const roomBed = (l.room_name ? infoEscape(l.room_name) : '-') + (l.bed_number ? ' / ' + infoEscape(l.bed_number) : '');
    html += `<tr>
...
function openLodgerModal(id) {
  ...
  const rooms = query('SELECT id, name FROM rooms ORDER BY name');
  const currentRoomId = l.bed_id ? (query('SELECT room_id FROM beds WHERE id = ?', [l.bed_id])[0]?.room_id || '') : '';
  ...
  const beds = currentRoomId ? query('SELECT id, bed_number, status FROM beds WHERE room_id = ? ORDER BY bed_number', [currentRoomId]) : [];
  const bedOptions = [['', '请选择床位'], ...beds.map(b => [b.id, b.bed_number])];
...
function infoReloadBedOptions(roomSelectId, bedSelectId, selectedBedId) {
  ...
  const beds = query('SELECT id, bed_number FROM beds WHERE room_id = ? ORDER BY bed_number', [roomId]);
...
async function submitLodger(id) {
  ...
  if (!checkIn || !expectedOut) {
    ...
    if (!expectedOut) infoShowFieldError('info-lodger-checkout', '请选择预离日期');
    return;
  }
  ...
  if (dup) {
    const infoDup = personDisplayName(dup) + (dup.phone ? ' · ' + dup.phone : '');
    if (!confirm(`检测到该手机号/身份证已有在住记录：${infoDup}\n是否继续保存？`)) return;
  }

  // 床位占用校验：新床位不能被其他在住住客占用
  if (bedId) {
    const other = query('SELECT id FROM lodgers WHERE bed_id = ? AND status = \'在住\' AND id <> ?', [bedId, id])[0];
    if (other) {
  ...
  if (status === '已退' && l.status === '在住') {
    ...
    if (balance > 0) {
      infoToast('该挂单尚有余额 ' + balance.toFixed(2) + ' 元，请使用「退房」功能处理退款');
      return;
    }
    actualOut = todayStr();
    finalBedId = null;
  ...
  try {
    await withTransaction(async () => {
      run(`UPDATE lodgers SET name=?, dharma_name=?, gender=?, phone=?, id_card=?,
        ...
        [name, person.dharma_name, gender, phone, idCard, checkIn, expectedOut, actualOut,
         status, source, finalBedId, notes, id]);

      // 床位状态同步
      const oldBedId = l.bed_id;
      if (oldBedId && oldBedId !== finalBedId) {
        const stillOccupied = query('SELECT COUNT(*) as c FROM lodgers WHERE bed_id = ? AND status = \'在住\' AND id <> ?', [oldBedId, id])[0]?.c || 0;
        if (stillOccupied === 0) {
          run("UPDATE beds SET status='可用' WHERE id=?", [oldBedId]);
          setHouseStatus(oldBedId, status === '已退' ? '脏房' : '可用', status === '已退' ? '挂单退床' : '挂单换床释放旧床位');
        }
      }
      ...
      // 同步用斋记录与住宿日期
      if (status === '在住') {
        run("DELETE FROM meals WHERE lodger_id = ? AND (date < ? OR date > ?)", [id, checkIn, expectedOut]);
        const defaults = getLodgerMealDefaults(id);
        await generateMeals(id, checkIn, expectedOut, defaults.breakfast, defaults.lunch, defaults.dinner);
      } else {
        const cutoff = actualOut || checkIn;
        run("DELETE FROM meals WHERE lodger_id = ? AND date > ?", [id, cutoff]);
      }
...
async function deleteInfoLodger(id) {
  ...
  try {
    await withTransaction(async () => {
      ...
      logAudit('删除挂单', 'lodger', id, { guest_id: l.guest_id, name: l.name, bed_id: l.bed_id });
