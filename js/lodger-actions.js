...
function renderBedActionMenu(lodgerId) {
  ...
  function item(action, iconName, label, extraCls) {
    return '<button type="button" class="bed-action-item ui-menu-item' + (extraCls ? ' ' + extraCls : '') + '" role="menuitem" onclick="event.stopPropagation(); closeBedActionMenus(); ' + action + '(' + id + ')">' +
      icon(iconName, 'icon-xs') +
      '<span>' + label + '</span>' +
    '</button>';
  }
  return '<div class="bed-action-menu" data-lodger-id="' + id + '" onclick="event.stopPropagation()">' +
    ...
    '<div class="bed-action-popover ui-menu" role="menu">' +
      ...
      item('deleteLodger', 'delete', '删除', 'bed-action-item-danger ui-menu-item-danger') +
    '</div>' +
  '</div>';
}
...
function openLodgerActions(id) {
  closeBedActionMenus();
  const menu = document.querySelector('.bed-action-menu[data-lodger-id="' + id + '"]');
  if (menu) {
    const trigger = menu.querySelector('.bed-action-trigger');
...
function openExtendModal(id) {
  const l = query("SELECT l.*, r.name as room_name, b.bed_number FROM lodgers l LEFT JOIN beds b ON b.id=l.bed_id LEFT JOIN rooms r ON r.id=b.room_id WHERE l.id=?", [id])[0];
  const label = escapeHtml((l.room_name || '-') + (l.bed_number ? ' / ' + l.bed_number : ''));
  const modal = document.getElementById('modal');
  document.getElementById('modal-title').textContent = '续住 - ' + escapeHtml(personDisplayName(l));
  setModalBody(`
...
async function submitExtend(id) {
  ...
  if (!date) { alert('请选择新的预离日期'); return; }
  ...
  try {
    ...
    await withTransaction(async () => {
      ...
      const existing = query("SELECT date FROM meals WHERE lodger_id=? ORDER BY date DESC LIMIT 1", [id])[0];
      const start = existing ? formatLocalDate(new Date(new Date(existing.date + 'T12:00:00').getTime() + 86400000)) : l.check_in_date;
...
function openChangeBedModal(id) {
  const l = query("SELECT l.*, r.name as room_name, b.bed_number, r.dorm_type FROM lodgers l LEFT JOIN beds b ON b.id=l.bed_id LEFT JOIN rooms r ON r.id=b.room_id WHERE l.id=?", [id])[0];
  const modal = document.getElementById('modal');
  document.getElementById('modal-title').textContent = '换床 - ' + escapeHtml(personDisplayName(l));
  setModalBody(`
    <div class="modal-form">
      <div class="modal-summary">
...
function selectChangeBed(e, bedId) {
  ...
  const el = document.querySelector('#chg-bed-dropdown .bp-bed[data-bed="' + bedId + '"]');
  let label = el && el.dataset.label ? el.dataset.label : String(bedId);
  if (hidden) hidden.value = bedId;
  if (labelEl) labelEl.textContent = label;
  document.querySelectorAll('#chg-bed-dropdown .bp-bed').forEach(function (row) {
...
async function submitChangeBed(lodgerId, gender) {
  ...
  if (!dormMatchGender(bed.dorm_type, gender)) {
    alert('该床位所在房间寮类型不符'); return;
  }
  const occ = query("SELECT COUNT(*) as c FROM lodgers WHERE bed_id=? AND status='在住'", [bedId])[0].c;
...
function openEditLodgerModal(id) {
  const l = query("SELECT * FROM lodgers WHERE id=?", [id])[0];
  const modal = document.getElementById('modal');
  document.getElementById('modal-title').textContent = '编辑挂单 - ' + escapeHtml(personDisplayName(l));
  setModalBody(`
    <div class="modal-form">
      <div class="form-grid">
...
async function submitEditLodger(id) {
  ...
  const bed = query(`SELECT b.*, r.dorm_type FROM beds b JOIN rooms r ON r.id=b.room_id WHERE b.id=?`, [l.bed_id])[0];
  if (!dormMatchGender(bed.dorm_type, gender)) {
    if (!confirm(`该床位所在房间为「${bed.dorm_type}」，修改性别后可能不符合安排。是否继续？`)) return;
  }
  ...
  try {
    ...
    await withTransaction(async () => {
      ...
      // 同步更新关联 guests 表
      const lodger = query("SELECT guest_id FROM lodgers WHERE id=?", [id])[0];
      if (lodger && lodger.guest_id) {
        run(`UPDATE guests SET
          name=?, dharma_name=?, gender=?, phone=?, id_card=?, updated_at=?
          WHERE id=?`, [
          person.name, person.dharma_name,
          gender || null, phone, idCard,
          new Date().toISOString(), lodger.guest_id
        ]);
      }

      logAudit('编辑挂单', 'lodger', id, { guest_id: lodger && lodger.guest_id, name: name });

      // 重新生成用斋记录（保留已有跳过设置；缺失日期按默认用斋）
      const existing = {};
      query("SELECT * FROM meals WHERE lodger_id=?", [id]).forEach(m => { existing[m.date] = m; });
      run("DELETE FROM meals WHERE lodger_id=?", [id]);
      const defaults = getLodgerMealDefaults(id);
      const dates = getLodgerStayDates({ check_in_date: checkIn, expected_check_out: checkOut });
      dates.forEach(d => {
        const m = existing[d] || { breakfast: defaults.breakfast, lunch: defaults.lunch, dinner: defaults.dinner };
        run(`INSERT INTO meals (lodger_id, date, breakfast, lunch, dinner) VALUES (?, ?, ?, ?, ?)`,
...
function openCheckoutModal(id) {
  const l = query("SELECT l.*, r.name as room_name, b.bed_number FROM lodgers l LEFT JOIN beds b ON b.id=l.bed_id LEFT JOIN rooms r ON r.id=b.room_id WHERE l.id=?", [id])[0];
  const label = escapeHtml((l.room_name || '-') + (l.bed_number ? ' / ' + l.bed_number : ''));
  const paid = query("SELECT COALESCE(SUM(CASE WHEN type IN ('押金','房费') THEN amount ELSE 0 END), 0) as income, COALESCE(SUM(CASE WHEN type = '退款' THEN amount ELSE 0 END), 0) as refund FROM payments WHERE lodger_id = ?", [id])[0];
  const balance = (paid.income || 0) - (paid.refund || 0);
  const modal = document.getElementById('modal');
  document.getElementById('modal-title').textContent = '退房 - ' + escapeHtml(personDisplayName(l));
  setModalBody(`
    <div class="modal-form">
      <div class="modal-summary">
...
async function submitCheckout(id) {
  ...
  if (refund < 0) { alert('退款金额不能为负数'); return; }
  const paid = query("SELECT COALESCE(SUM(CASE WHEN type IN ('押金','房费') THEN amount ELSE 0 END), 0) as income, COALESCE(SUM(CASE WHEN type = '退款' THEN amount ELSE 0 END), 0) as refund_total FROM payments WHERE lodger_id = ?", [id])[0];
  const balance = (paid.income || 0) - (paid.refund_total || 0);
  if (refund > balance) { alert(`退款金额不能超过余额 ${balance.toFixed(2)}`); return; }
  ...
  try {
    ...
    await withTransaction(async () => {
      ...
      if (refund > 0) {
        run("INSERT INTO payments (lodger_id, type, amount, method, remark) VALUES (?, '退款', ?, ?, ?)",
          [id, refund, method, notes]);
        logAudit('退款', 'lodger', id, { guest_id: l.guest_id, name: l.name, refund: refund, method: method });
      } else {
        // 零退款也写入一条占位流水，确保每笔退房都有支付记录可追踪
        run("INSERT INTO payments (lodger_id, type, amount, method, remark) VALUES (?, '退款', 0, ?, ?)",
          [id, method, '退房结算（无退款）']);
      }
...
function printVoucher(id) {
  ...
  `, [id])[0];
  const modal = document.getElementById('modal');
  document.getElementById('modal-title').textContent = '挂单凭证 - ' + escapeHtml(personDisplayName(l));
