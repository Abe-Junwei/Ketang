document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (typeof upgradeSelects === 'function') upgradeSelects(document);
    window.ketangReady = true;
    console.log('客堂系统初始化完成');
  } catch (e) {
    console.error('初始化失败 | Init failed:', e);
    document.querySelectorAll('.sidebar-nav-btn, .sidebar-footer-btn').forEach(b => { b.disabled = true; b.style.opacity = '0.5'; });
...
function updateTopbarTitle(name) {
  ...
  if (search) search.parentElement.style.display = (name === 'board' || name === 'lodging') ? '' : 'none';
}
...
function showView(name) {
  ...
  document.querySelectorAll('.sidebar-nav-btn, .sidebar-footer-btn').forEach(b => b.classList.remove('active'));
  document.getElementById('view-' + name).classList.add('active');
  const navView = name === 'stay' ? 'stay' : name;
  const primary = document.querySelector('.sidebar-nav-btn[data-view="' + navView + '"]');
  const footer = document.querySelector('.sidebar-footer-btn[data-view="' + navView + '"]');
  ...
  if (name === 'forecast') { initForecastDates(); renderForecastTab('today'); }
...
function getBoardBedStats() {
  ...
  var total = query("SELECT COUNT(*) as c FROM beds b JOIN rooms r ON r.id = b.room_id WHERE b.status != '备用' AND " + spareSql)[0]?.c || 0;
  var occupied = query("SELECT COUNT(DISTINCT l.bed_id) as c FROM lodgers l JOIN beds b ON b.id = l.bed_id JOIN rooms r ON r.id = b.room_id WHERE l.status='在住' AND b.status != '备用' AND " + spareSql)[0]?.c || 0;
  var empty = Math.max(0, total - occupied);
  var dirty = query("SELECT COUNT(*) as c FROM beds b JOIN rooms r ON r.id = b.room_id WHERE b.status != '备用' AND " + spareSql + " AND (SELECT status FROM housekeeping WHERE bed_id = b.id ORDER BY changed_at DESC LIMIT 1) = '脏房'")[0]?.c || 0;
  var cleanEmpty = Math.max(0, empty - dirty);
  var lodgerCount = query("SELECT COUNT(*) as c FROM lodgers WHERE status='在住'")[0]?.c || 0;
  var resvToday = query("SELECT COUNT(*) as c FROM reservations WHERE expected_check_in = ? AND status IN ('预约','已确认')", [todayStr()])[0]?.c || 0;
  var occPct = total ? Math.round(occupied / total * 100) : 0;
  return { total: total, occupied: occupied, empty: empty, dirty: dirty, cleanEmpty: cleanEmpty, lodgerCount: lodgerCount, resvToday: resvToday, occPct: occPct };
}
...
function renderBoardCharts() {
  ...
  var expArrive = query("SELECT COUNT(*) as c FROM (SELECT id FROM reservations WHERE expected_check_in = ? AND status IN ('预约','已确认') UNION ALL SELECT id FROM lodgers WHERE check_in_date = ? AND status = '在住')", [today, today])[0].c;
  var expDepart = query("SELECT COUNT(*) as c FROM lodgers WHERE expected_check_out = ? AND status = '在住'", [today])[0].c;
  var actArrive = query("SELECT COUNT(*) as c FROM lodgers WHERE check_in_date = ? AND status = '在住'", [today])[0].c;
  var actDepart = query("SELECT COUNT(*) as c FROM lodgers WHERE actual_check_out = ?", [today])[0].c;
  ...
  var spareSql = spareRoomExcludeClause('r');
  var maleBeds = query("SELECT COUNT(*) as c FROM beds b JOIN rooms r ON r.id=b.room_id WHERE r.dorm_type='男寮' AND b.status!='维修' AND b.status!='备用' AND " + spareSql)[0]?.c || 0;
  var femaleBeds = query("SELECT COUNT(*) as c FROM beds b JOIN rooms r ON r.id=b.room_id WHERE r.dorm_type='女寮' AND b.status!='维修' AND b.status!='备用' AND " + spareSql)[0]?.c || 0;
  var maleOcc = query("SELECT COUNT(DISTINCT l.bed_id) as c FROM lodgers l JOIN beds b ON b.id=l.bed_id JOIN rooms r ON r.id=b.room_id WHERE l.status='在住' AND r.dorm_type='男寮' AND b.status!='备用' AND " + spareSql)[0]?.c || 0;
  var femaleOcc = query("SELECT COUNT(DISTINCT l.bed_id) as c FROM lodgers l JOIN beds b ON b.id=l.bed_id JOIN rooms r ON r.id=b.room_id WHERE l.status='在住' AND r.dorm_type='女寮' AND b.status!='备用' AND " + spareSql)[0]?.c || 0;
  ...
  if (dormSub) dormSub.textContent = '男余 ' + maleFree + ' · 女余 ' + femaleFree;
  ...
  createKetangChart('board-dorm', 'chart-board-dorm', {
    ...
    data: {
      labels: ['男寮', '女寮'],
      datasets: [
        { label: '在住', data: [maleOcc, femaleOcc], backgroundColor: [T.male, T.female], borderRadius: 6, borderSkipped: false, maxBarThickness: 36 },
        { label: '余床', data: [maleFree, femaleFree], backgroundColor: [T.successSoft, T.primarySoft], borderRadius: 6, borderSkipped: false, maxBarThickness: 36 }
      ]
    },
    options: {
      ...
      scales: {
        ...
        y: { stacked: true, beginAtZero: true, ticks: { stepSize: 1, precision: 0 }, grid: { drawBorder: false } }
      }
    }
  });
...
function renderLodgingOccupancyChart() {
  if (typeof Chart === 'undefined') return;
  renderBoardRingChart('lodging-occ', 'chart-lodging-occ', 'lodging-occ-pct', getBoardBedStats());
}
...
function renderAll() {
  ...
  if (document.getElementById('view-forecast')?.classList.contains('active')) {
    var activeTab = document.querySelector('.forecast-tab-btn.active')?.dataset.tab || 'today';
...
function handleLodgerSearch(q) {
  ...
  hint.hidden = false;
  hint.textContent = visible ? '匹配 ' + visible + ' 人' : '未找到匹配的在住挂单';
}
...
function updateLodgersPageMeta() {
  ...
  if (!countEl) return;
  var total = query("SELECT COUNT(*) as c FROM lodgers WHERE status='在住'")[0]?.c || 0;
...
function renderStats() {
  ...
  if (!strip) return;
  const emptyPct = stats.total ? Math.round(stats.empty / stats.total * 100) : 0;
  strip.innerHTML = `
...
function renderBedDetailRow(b, idx) {
  ...
  if (b.lodger_id) {
    ...
    return '<div class="room-detail-row room-detail-row-occ">' +
      '<div class="room-detail-row-main">' + bedTag +
        '<span class="room-detail-row-text">' +
          '<span class="room-detail-dot room-detail-dot-occ" aria-hidden="true">●</span>' +
          '<span class="room-detail-guest">' + escapeHtml(personDisplayName(b)) + '</span>' +
        '</span>' +
      '</div>' + actionHtml +
    '</div>';
  }
  if (b.status === '维修') {
    return '<div class="room-detail-row room-detail-row-dirty">' +
      '<div class="room-detail-row-main">' + bedTag +
  ...
  actionHtml = '<button type="button" class="bed-action-trigger bed-action-trigger-icon bed-action-trigger-assign" title="入住" aria-label="入住" onclick="event.stopPropagation(); openCheckinForBed(' + b.id + ')">' + icon('assign', 'icon-xs') + '</button>';
  return '<div class="room-detail-row room-detail-row-free">' +
    '<div class="room-detail-row-main">' + bedTag +
      '<span class="room-detail-row-text room-detail-row-muted">' +
...
function renderRoomDetailPanel(roomId, cardEl) {
  ...
  let offset = 0;
  const columnsHtml = columns.map(function (colBeds) {
    let rows = colBeds.map(function (b, i) {
...
function renderRooms() {
  ...
  rooms.forEach(function (r) {
    ...
    groups[key].push(r);
    if (locFloor[key] === undefined || r.floor > locFloor[key]) locFloor[key] = r.floor;
  });
  ...
  locOrder.forEach(function (loc) {
    ...
    sectionHtml += '<div class="board-loc-title-row"><h3 class="board-loc-title">' +
      escapeHtml(loc) + ' · ' + rlist.length + ' 间</h3></div>';
    ...
    rlist.forEach(function (r) {
      ...
      const occBeds = beds.filter(function (b) { return b.lodger_id; }).length;
      let dirtyBeds = beds.filter(function (b) { return !b.lodger_id && b.hk_status === '脏房'; }).length;
      const maintBeds = beds.filter(function (b) { return b.status === '维修'; }).length;
      let cardTheme = 'empty';
      ...
      const cleanIcon = (dirtyBeds > 0 || maintBeds > 0)
        ? '<span class="room-card-clean-icon" aria-label="脏房或维护">' + icon('cleaning', 'icon-sm') + '</span>'
        : '';

      const nameCls = cardTheme === 'empty' ? ' room-card-name-muted' : '';

      const miniDots = beds.map(function (b, idx) {
        ...
        return '<span class="room-bed-dot ' + dotCls + '" title="' + escapeHtml(tip) + '"></span>';
      }).join('');
      ...
      const searchAttr = escapeHtml(searchParts.filter(Boolean).join(' ').toLowerCase());
      ...
      const bodyHtml =
        '<div class="room-card-meta-row">' +
          '<div class="room-bed-dots' + dotsClass + '" aria-hidden="true">' + miniDots + '</div>' +
          metaRight +
        '</div>';

      const dormCls = r.dorm_type === '男寮' ? ' room-card-dorm-male'
        : r.dorm_type === '女寮' ? ' room-card-dorm-female' : '';

      sectionHtml += '<div class="room-card room-card-' + cardTheme + dormCls + '" data-search="' + searchAttr + '" onclick="toggleRoomExpand(' + r.id + ', this)">' +
        '<div class="room-card-header">' +
          '<span class="room-card-name' + nameCls + '" title="' + escapeHtml(r.name) + '">' + escapeHtml(r.name) + '</span>' +
        '</div>' +
  ...
  grid.innerHTML = html || '<p class="empty-tip">暂无房间数据，请先在基础设置中维护房号。</p>';
}
...
function renderLodgers() {
  ...
  lodgers.forEach(function (l, idx) {
    ...
    const roomLabel = escapeHtml((l.room_name || '-') + (l.bed_number ? ' - ' + bedLabel : ''));
    const searchParts = [personDisplayName(l), l.gender, l.phone, l.role, l.room_name, l.location, l.bed_number, l.check_in_date, l.expected_check_out];
    const tr = document.createElement('tr');
    tr.setAttribute('data-search', searchParts.filter(Boolean).join(' ').toLowerCase());
    tr.innerHTML = `
  ...
  if (lodgers.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7" class="empty-tip">暂无在住挂单</td></tr>';
  }
  updateLodgersPageMeta();
}
...
function renderOpsNotice() {
  ...
  `, [today, today])[0].c;
  const departures = query("SELECT COUNT(*) as c FROM lodgers WHERE expected_check_out = ? AND status = '在住'", [today])[0].c;
  ...
  if (alerts.length > 0) {
    ...
    const gap = first.expected_count - (first.checked_in || 0) - (first.reserved || 0);
  ...
  const label = escapeHtml(first.room_name) + (first.bed_number ? ' ' + escapeHtml(formatBedLabel(first.bed_number, 0)) : '');
...
function switchReminderTab(key) {
  ...
  listEl.innerHTML = rows.map(function (r, i) {
    const dormLabel = r.dorm_type === '男寮' ? '男寮' : r.dorm_type === '女寮' ? '女寮' : escapeHtml(r.dorm_type || '');
    const room = escapeHtml(r.room_name || '-');
    const timeHint = key === 'today' ? '<span class="reminder-time">' + (i % 2 === 0 ? '12:00 前' : '14:00 前') + '</span>' : '';
    return '<div class="reminder-row">' +
      '<span><strong>' + escapeHtml(personDisplayName(r)) + '</strong> <span class="reminder-meta">(' + dormLabel + ' ' + room + ')</span></span>' +
      '<span class="reminder-row-actions">' + timeHint + renderBedActionMenu(r.id) + '</span>' +
    '</div>';
  }).join('');
}
...
function renderCheckoutReminders() {
  ...
  tabsEl.innerHTML = groups.map(function (g, i) {
    ...
    const overdueCls = g.key === 'overdue' && count > 0 ? ' tab-btn-danger' : '';
    const activeBadgeCls = i === 0 ? ' tab-badge-active' : '';
    return '<button type="button" class="tab-btn' + (i === 0 ? ' active' : '') + overdueCls + '" role="tab" data-tab="' + g.key + '" aria-selected="' + (i === 0 ? 'true' : 'false') + '" onclick="switchReminderTab(\'' + g.key + '\')">' +
      escapeHtml(g.label) + ' <span class="tab-badge' + activeBadgeCls + '">' + count + '</span></button>';
  }).join('');
    ...
    setModalBody(
      '<div class="modal-form confirm-dialog">' +
        ...
        '<div class="modal-actions">' +
          '<button type="button" class="btn btn-default" id="confirm-cancel">' + escapeHtml(cancelText) + '</button>' +
          '<button type="button" class="btn ' + (danger ? 'btn-danger' : 'btn-primary') + '" id="confirm-ok">' + escapeHtml(confirmText) + '</button>' +
        '</div>' +
      '</div>'
    );
    document.getElementById('confirm-cancel').onclick = function () { dismissConfirm(false); };
    document.getElementById('confirm-ok').onclick = function () { dismissConfirm(true); };
