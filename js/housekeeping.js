function getHouseStatus(bedId) {
  const row = query("SELECT * FROM housekeeping WHERE bed_id = ? ORDER BY changed_at DESC LIMIT 1", [bedId])[0];
  return row ? row.status : '净房';
}


function setHouseStatus(bedId, status, notes, operator) {
  run("INSERT INTO housekeeping (bed_id, status, operator, notes) VALUES (?, ?, ?, ?)",
...
function isBedAssignable(bedId) {
  ...
  const occ = query("SELECT COUNT(*) as c FROM lodgers WHERE bed_id = ? AND status = '在住'", [bedId])[0]?.c || 0;
...
function renderHousekeeping() {
  ...
  rooms.forEach(r => {
    ...
    beds.forEach(b => {
      ...
      div.className = 'room ' + (hk === '脏房' ? 'partial' : (occupied ? 'full' : 'empty'));
      div.innerHTML = `
  ...
  try {
    ...
    await withTransaction(async () => {
      ...
      if (status === '维修') run("UPDATE beds SET status='维修' WHERE id=?", [bedId]);
        ...
        if (occ === 0) run("UPDATE beds SET status='可用' WHERE id=?", [bedId]);
      }
