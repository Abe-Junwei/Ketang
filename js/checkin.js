function populateEventSelect(selectId, selectedId) {
  ...
  if (!sel) return;
  const html = '<option value="">散客 / 不归属营期</option>' + getEventOptionsHtml(selectedId, false);
...
function renderBedPicker(options) {
  ...
  rooms.forEach(function (r) {
    if (spareRoomFilter && typeof isSpareRoom === 'function' && isSpareRoom(r)) return;
    ...
    const expanded = selectedRoomId && r.id == selectedRoomId ? ' expanded' : '';
    ...
    if (!isFull) {
      ...
      beds.forEach(function (b) {
        ...
        hasBeds = true;
        const bedLabel = escapeHtml((b.bed_number || '') + ' · ' + (b.hk_status || ''));
        const selClass = (selectedBedId && b.id == selectedBedId) ? ' selected' : '';
        html += `<div class="bp-bed${selClass}" data-bed="${b.id}" data-label="${bedLabel}" onclick="${onSelect}(event, ${b.id})">`;
        html += escapeHtml(b.bed_number);
        html += '<span class="bp-bed-status">' + escapeHtml(b.hk_status) + '</span>';
        html += '</div>';
      });
  ...
  dropdown.innerHTML = hasAnyRoom
    ? html
    : '<div class="bp-empty">' + (gender ? '没有符合性别要求的可用房间' : '暂无可选房间') + '</div>';
}
...
function renderBedOptions(selectedRoomId, selectedBedId) {
  ...
  if (selectedBedId) {
    const selBed = query('SELECT b.*, r.name as room_name FROM beds b JOIN rooms r ON r.id=b.room_id WHERE b.id=?', [selectedBedId])[0];
    if (selBed) {
...
function toggleRoom(e, roomId) {
  ...
  if (!room && roomId) room = document.querySelector('.bp-room[data-room="' + roomId + '"]');
...
function selectBed(e, bedId) {
  ...
  document.querySelectorAll('.bp-bed').forEach(function (row) { row.classList.remove('selected'); });
...
function openNewCheckinForBed(bedId) {
  ...
  showView('checkin');
  renderBedOptions();
  const bed = query('SELECT b.*, r.name as room_name FROM beds b JOIN rooms r ON r.id=b.room_id WHERE b.id=?', [bedId])[0];
  if (bed) {
    document.getElementById('ci-bed').value = bedId;
...
function renderAssignPickRow(kind, id, bedId, title, meta) {
  const fn = kind === 'lodger' ? 'assignExistingLodgerToBed' : 'assignReservationToBed';
  return '<button type="button" class="assign-pick-item ui-menu-item" onclick="' + fn + '(' + id + ',' + bedId + ')">' +
    '<span class="assign-pick-text">' +
      '<span class="assign-pick-name">' + title + '</span>' +
      '<span class="assign-pick-meta">' + meta + '</span>' +
    '</span>' +
  '</button>';
}
...
function renderAssignPickSection(title, emptyTip, rowsHtml) {
  return '<div class="assign-pick-section">' +
    '<div class="assign-pick-section-title">' + escapeHtml(title) + '</div>' +
    '<div class="assign-pick-list">' +
      (rowsHtml || ('<p class="empty-tip assign-pick-empty">' + escapeHtml(emptyTip) + '</p>')) +
    '</div>' +
  '</div>';
}
...
function openAssignBedModal(bedId) {
  const bed = query('SELECT b.*, r.name as room_name, r.dorm_type FROM beds b JOIN rooms r ON r.id=b.room_id WHERE b.id=?', [bedId])[0];
  if (!bed) return;
  ...
  `).filter(function (l) { return dormMatchGender(bed.dorm_type, l.gender); });

  const reservations = query(`
    SELECT * FROM reservations
    WHERE status IN ('预约', '已确认')
    ORDER BY expected_check_in ASC, id ASC
  `).filter(function (r) { return dormMatchGender(bed.dorm_type, r.gender); });

  const lodgerRows = lodgers.map(function (l) {
    ...
    return renderAssignPickRow('lodger', l.id, bedId, title, escapeHtml(metaParts.join(' · ') || '已登记'));
  }).join('');

  const resvRows = reservations.map(function (r) {
    ...
    return renderAssignPickRow('reservation', r.id, bedId, title, escapeHtml(metaParts.join(' · ') || '预约'));
  }).join('');
  ...
  setModalBody(
    '<div class="modal-form assign-bed-modal">' +
      '<div class="modal-summary">' +
        '<p><span class="modal-summary-label">目标床位</span>' + bedLabel + '</p>' +
        '<p class="modal-summary-muted">优先选择已登记或预约待入住的客人；也可新建登记。</p>' +
      '</div>' +
      renderAssignPickSection('已登记 · 未分床', '暂无已登记未分床的客人', lodgerRows) +
      renderAssignPickSection('预约 · 待入住', '暂无待入住预约', resvRows) +
      '<div class="modal-actions">' +
        '<button type="button" class="btn btn-default" onclick="closeModal()">取消</button>' +
        '<button type="button" class="btn btn-primary" onclick="openNewCheckinForBed(' + bedId + ')">新建登记</button>' +
      '</div>' +
    '</div>'
  );
...
async function assignExistingLodgerToBed(lodgerId, bedId) {
  const l = query("SELECT * FROM lodgers WHERE id=? AND status='在住'", [lodgerId])[0];
  if (!l) { alert('挂单不存在或已不在住'); return; }
  if (l.bed_id) { alert('该挂单已有床位'); return; }
  const bed = query('SELECT b.*, r.dorm_type FROM beds b JOIN rooms r ON r.id=b.room_id WHERE b.id=?', [bedId])[0];
  if (!bed) return;
...
async function assignReservationToBed(resvId, bedId) {
  ...
  try {
    await withTransaction(async () => {
      ...
      await generateMeals(lodgerId, checkIn, checkOut, mf.breakfast, mf.lunch, mf.dinner);
      logAudit('预约分配床位', 'lodger', lodgerId, { guest_id: guestId, bed_id: bedId, reservation_id: resvId, name: r.name });
...
document.getElementById('checkin-form').addEventListener('submit', async e => {
  ...
  if (checkOut && checkOut < checkIn) { alert('预离日期不能早于入住日期'); return; }
  ...
  try {
    await withTransaction(async () => {
      ...
      // 更新客人紧急联系人信息
      const emergencyName = document.getElementById('ci-emergency-name').value.trim() || null;
      const emergencyPhone = document.getElementById('ci-emergency-phone').value.trim() || null;
      if (emergencyName || emergencyPhone) {
        run("UPDATE guests SET emergency_contact = COALESCE(?, emergency_contact), emergency_phone = COALESCE(?, emergency_phone), updated_at = ? WHERE id = ?",
      ...
      const roomFee = parseFloat(document.getElementById('ci-room-fee').value) || 0;
      const payMethod = document.getElementById('ci-pay-method').value || null;
      const payRemark = document.getElementById('ci-pay-remark').value.trim() || null;
      if (deposit > 0) {
        run("INSERT INTO payments (lodger_id, type, amount, method, remark) VALUES (?, '押金', ?, ?, ?)",
          [lodgerId, deposit, payMethod, payRemark]);
      }
      if (roomFee > 0) {
        run("INSERT INTO payments (lodger_id, type, amount, method, remark) VALUES (?, '房费', ?, ?, ?)",
          [lodgerId, roomFee, payMethod, payRemark]);
      }
      if (deposit > 0 || roomFee > 0) {
        logAudit('收款', 'lodger', lodgerId, { guest_id: guestId, name: name, deposit: deposit, room_fee: roomFee, method: payMethod });
      }

      logAudit('入住登记', 'lodger', lodgerId, { guest_id: guestId, bed_id: bedId, name: name });

      // 若从预约转入住，更新预约状态
      if (resvId) {
        const rsvNow = query("SELECT status FROM reservations WHERE id=?", [resvId])[0];
...
function parseCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    rows.push({
      ...
      expected_check_out: get(cols, ['预离日期', 'expected_check_out', 'checkOut']),
      event_name: get(cols, ['营期', 'event', '团体批次', 'group_code', 'group']),
...
function findAssignableBed(gender, roomPreference) {
  ...
  if (roomPreference) {
    ...
    for (const b of exact) {
      ...
      if ((hk === '净房' || hk === '可用') && dormMatchGender(b.dorm_type, gender)) return b;
    }
  }
  // 按性别匹配
  const beds = query(`
...
async function importBatchCSV(input) {
  ...
  reader.onload = async e => {
    try {
      ...
      if (rows.length === 0) { resultDiv.innerHTML = '<p class="empty-tip">未读取到有效数据</p>'; return; }

      if (!validateMealNeedPicker('batch-meal-need')) {
        resultDiv.innerHTML = '<p class="field-error">请选择用斋默认（至少选一餐）</p>';
        return;
      }
      ...
      let success = 0, fail = 0, failedRows = [];

      for (let idx = 0; idx < rows.length; idx++) {
        const row = rows[idx];
        try {
          if (!row.name || !row.gender) { throw new Error('姓名/性别缺失'); }
          ...
          const evt = row.event_name ? (findEventByName(row.event_name) || null) : null;
          const guestId = findOrCreateGuest(row.name, row.gender, row.phone || null, row.id_card || null);
          incrementGuestVisit(guestId, checkIn);
          ...
          await withTransaction(async () => {
            ...
            logAudit('批量导入入住', 'lodger', lodgerId, { guest_id: guestId, bed_id: bed.id, name: row.name });
          });
          success++;
        } catch (err) {
          fail++;
          failedRows.push(`第 ${idx + 2} 行（${row.name || ''}）：${err.message}`);
        }
      }
      ...
      resultDiv.innerHTML = '<p style="color:var(--color-danger)">导入出错：' + escapeHtml(err.message) + '</p>';
    }
  };
...
function downloadBatchTemplate() {
  const headers = ['姓名/法名', '性别', '手机号', '身份证', '身份', '入住日期', '预离日期', '营期', '班级', '房间偏好', '备注'];
  const sample = ['张三', '男', '13800138000', '', '客人', todayStr(), '', '2026-06-水陆法会', '一班', '男众', ''];
