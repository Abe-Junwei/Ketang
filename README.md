...
> 面向寺院客堂的在线住宿/挂单管理系统。正式形态部署在 Cloudflare 上，支持多人同时使用；本地文件夹形态仅用于开发、演示、灾备和离线恢复。

## 一、产品定位

- **不采购**：自研，代码可控。
- **正式形态**：Cloudflare Pages + Functions + D1，寺院内部人员使用账号登录，多人共享同一份数据。
- **本地后备**：`sql.js` + IndexedDB + `ketang.db`，用于无网络演示、开发调试或灾难恢复。
- **免安装桌面包**：Windows 上双击 `启动客堂系统.bat` 或 macOS 双击 `客堂管理系统.app` 即可用 Chrome/Edge 打开本地版。
