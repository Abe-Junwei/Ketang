...
> 面向寺院客堂的**在线**住宿/挂单管理系统，部署在 Cloudflare（D1 + Pages），支持多人同时使用。  
> 单机便携 / `file://` 本地库模式已停用；开发请用 `scripts/dev_server.py` 代理到已部署环境。

## 一、产品定位

- **不采购**：自研，代码可控。
- **正式形态**：Cloudflare Pages + Functions + D1，寺院内部人员使用账号登录，多人共享同一份数据。
- **在线权威**：Cloudflare D1 + `/api/v1`；浏览器 sql.js 为读缓存（**信息管理已改为 read-module API 直读**，其余页面逐步迁移）。
- **开发**：`python3 scripts/dev_server.py` → `http://127.0.0.1:8080`，API 代理到 `https://wulingkt.net`。
- **备份**：管理员导出 JSON；不再以本地 `ketang.db` 为主路径。
...
├── 客堂管理系统.app          # macOS 双击启动（不弹 Terminal）
...
1. 用 Safari（iOS）或 Chrome（Android）打开寺院在线网址并登录。
2. 按浏览器提示「添加到主屏幕」/「安装应用」。
3. 从桌面图标打开后可全屏使用；详见 `使用说明.txt` 第十八节。
4. 首次在线访问后会缓存页面资源；断网时仅能打开界面，**不能**代替正式云端办理。
...
- **不弹 Terminal**：双击 `客堂管理系统.app`（内部调用 `.command` 脚本）。
